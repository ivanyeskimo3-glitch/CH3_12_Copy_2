...
from django.forms.models import model_to_dict #轉成字典
...
def search_list(request):
     ...
     # resultObject = students.objects.all().order_by("cID") # 建立物件
     # resultObject = students.objects.all().order_by("-cID") # 如果加個負號 代表反向排序
...
def index(request):
     ...
     data_count = len(resultList)# 要判斷有幾筆資料
     # print(data_count)
     # for d in resultList:
     #      print(model_to_dict(d))
...
from django.shortcuts import redirect # 重新導向    
def post(request):     
     if request.method == "POST": # 表單送出
          ...
          print(f"姓名:{cName},性別:{cSex},生日:{cBirthday},Email:{cEmail},電話:{cPhone},地址:{cAddr}")
          # return HttpResponse("資料已新增")
          add = students(cName=cName,cSex=cSex,cBirthday=cBirthday,
                         cEmail=cEmail,cPhone=cPhone,cAddr=cAddr)
          add.save()
          return redirect("/index/") #重新導向到/index/頁面
     else:

          return render(request, "post.html",locals()) # 顯示表單
...
def edit(request,id):
     if request.method == "POST":
          ...
          # print(id) # 取得網址列的id參數  
          obj_data = students.objects.get(cID=id) # 取得該筆資料
          print(model_to_dict(obj_data)) # 轉成字典顯示
